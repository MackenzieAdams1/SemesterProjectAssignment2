...
gage_fish_all <-
  ...
  mutate(site_name = paste0(str_extract(population_name, "^\\w+"), "_River"), .before = "stock_number") %>% # the format of this line makes it vulnerable to changes in the gage ids. Should probably be fixed but ehhhhhhhhhh......
...
gage_fish_all %>%
  ...
  theme(strip.text = element_text(size = 12), axis.title.x = element_text(size = 14), 
...
The plot we created shows year on the x axis and Discharge on the Y axis for the three rivers we are examining. This graph serves as an initial data exploration to examine patterns in discharge, potential flood events, and how much discharge is normal for each stream. There do not appear to be any significant gaps in our data set for the period we are examining (1987-2023). However, as seen in the Skokomish graph around 2010 the discharge dropped substantially and has remained this way ever since, with the discharge increasingly slightly in 2022 and 2023. Upon initial observation the dams on the Skokomish are not new, however, there was a lawsuit that occurred between the Skokomish Tribe and the State of Washington regarding discharge from the dams into the north fork of Skokomish which is located upstream of the gage we are using. Therefore, more research needs to be done to explore potential causes of this change. 
...
```{r}
#This histogram shows the frequency of different salmon return abundances for each of the three rivers we are analyzing. The scales are set to free so that the patterns of returns for each river can be adequately analyzed.
temp_1 <-
  gage_fish_all %>%
  reframe(abd_qty = mean(abundance_qty, na.rm = T), .by = ann)

gage_fish_all %>%
  summarise(abd_qty = mean(abundance_qty, na.rm = T), .by = ann, site_name) %>%
  ggplot(aes(x = abd_qty, fill = site_name, group = ann)) +
  geom_histogram() +
...
ANSWER:
The above plot tells us that annual salmon returns on the Dungeness River typically fall within 20-750 salmon, whereas annual returns on the Elwha and the Skokomish typically fall within 0-3,000 salmon. This data is as expected with a majority of salmon returns falling into the above defined parameters with some other larger runs occurring in all three rivers across the time of the data set. We need to consider the distribution of data when deciding on an analysis method as some methods assume a specific distribution is present in the data and if the data does not meet these assumptions the results that are produced may be misleading. Therefore, by assessing the distribution educated decisions can be made regarding what types of analysis methods would be the most appropriate. 
