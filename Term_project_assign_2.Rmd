...
all_gages <- c(Dungeness_Gage, Elwha_Gage, Skokomish_Gage)

start <- date("1987-10-01")
end <- date("2023-9-30")
para <-("00060")

Dungeness_Stream_df <- readNWISdv(siteNumbers = Dungeness_Gage, parameterCd = para, startDate = start, endDate = end) %>%
  renameNWISColumns() %>%
  mutate(
    rolling_mean = rollmean(Flow, 7, fill = NA, align = "right")
  )
  
Elwha_Stream_df <- readNWISdv(siteNumbers = Elwha_Gage, parameterCd = para, startDate = start, endDate = end) %>%
  renameNWISColumns()
Skokomish_Stream_df <- readNWISdv(siteNumbers = Skokomish_Gage, parameterCd = para, startDate = start, endDate = end) %>%
  renameNWISColumns()

#for loop to automate the above actions. This way, project is expandable. 
#future use: just have to add new rivers to all_gages list

for (gage in all_gages){
  temp_df <-
    readNWISdv(siteNumbers = gage, parameterCd = para, startDate = start, endDate = end) %>%
    renameNWISColumns() %>%
    mutate(
    rolling_mean = rollmean(Flow, 7, fill = NA, align = "right")
    )
  
  temp_name <- paste(str_extract(deparse(substitute(gage)), "^[^_]+"), "_steam_df")
