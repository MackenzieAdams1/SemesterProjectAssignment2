...
```{r}
## Name the gages you want here, and assign them their gage number. Add them all to "all_gages" as well
...
all_gages <- c(Dungeness_Gage, Elwha_Gage, Skokomish_Gage) %>%
  names(deparse(substitute(c(Dungeness_Gage, Elwha_Gage, Skokomish_Gage))))
...
## for loop to automate creating new dfs. This way, project is expandable. 
...
for (gage in all_gages){
  temp_df <-
    ...
    renameNWISColumns() %>%
    mutate(
    rolling_weekly = rollmean(Flow, 7, fill = NA, align = "right")
    )
  
  temp_name <- paste0(str_extract(all_gages[gage], "^[^_]+"), "_stream_df")
  ...
  if (first_time){
    ...
    all_dfs[[temp_name]] <- temp_df
  }
}


str_extract(all_gages[3], "^[^_]+")
