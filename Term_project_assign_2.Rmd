...
# Downloading data mostly works. Now to append it...
...
#unfucker that required an index number. Lets leave it sealed away but still in the code jic

# daymet_unfucker_hum_idx <-
#   function(f, index) {
#     print(str(f[[index]]))
#     bind_cols(data = f[[index]]$data, site = f[[index]]$site, .name_repair = "unique")
#   } 
...
gage_data <- 
  ...
  bind_rows() %>%
  select(site = site, 
         date = as.Date(yday-1, origin = "1987-01-01"),
         prcp_daily_mm = prcp..mm.day., 
         t_max_C = tmax..deg.c., 
         t_min_C = tmax..deg.c.)

  
gage_data <- lapply(readRDS("./DAYMET_all.rdata"), daymet_unfucker_hum) %>% bind_rows()
str(gage_data) # Examine the structure of gage_data
summary(gage_data$yday) # Summarize the yday column. Look for non-numeric values like NA, characters, or factors.
head(gage_data$yssday) # Look at the first few values of yday
unique(gage_data$yday) # Show all unique values in yday to identify non-numeric entries.


peepee <- lapply(readRDS("./DAYMET_all.rdata"), daymet_unfucker_hum) %>%
  bind_rows()
...
as.Date(peepee$yday, origin = "1987-01-01")
```
...
daymet_unfucker_hum <-
  function(f, index) {
    f[[index]]$data %>%
      bind_cols(site = f[[index]]$site, .name_repair = "unique") %>%
      return()
  }

# Chat's
daymet_unfucker_rob <- function(f, index = 1) { #Added index parameter
  
  data <- f[[index]]$data
  site <- f[[index]]$site
  
  result <- dplyr::bind_cols(data, site = site)
  print(result)
  return(result)
}

daymet_unfucker_hum(t2_list, 1)
...
tmin_deg_c_portangeles_gg <- skokomish_weather_df %>%
  filter(measurement == "tmin..deg.c.") %>%
  ggplot(aes(x = date, y = value)) +
  geom_line(color = "blue") +
  theme_linedraw() +
