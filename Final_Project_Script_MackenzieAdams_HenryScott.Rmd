...
anim_fish_all_dfll_df <- gage_fish_all %>%
  mutate(
    year = year(Date),
    month_day = as.Date(format(Date, "2000-%m-%d")), # Rebase all dates to year 2000
    title_clean = gsub("_", " ", site_name) # Corrected the missing parenthesis
  )


flow_anim <- 
  ggplot(anim_fish_all_df, aes(x = month_day, y = Flow, color = title_clean)) +
  geom_line() +
