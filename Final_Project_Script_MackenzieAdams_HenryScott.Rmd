...
```{r}
#Offsetting the climate data by four years (ex. salmon returns that occured during 1990's water year are now dependent on 1986 climate data) and assigning the salmon return values to change at each new water year.
...
aligned_data_stats <-
  ...
  group_by(site_name, waterYear) %>%
  reframe(abundance_qty = Mode(abundance_qty),
          Flow_median = median(Flow, na.rm = T),
          Flow_max = max(Flow, na.rm = T),
          Flow_min = min(Flow, na.rm = T),
          t_ann_median = median(t_avg_C, na.rm = T),
          t_ann_min = min(t_avg_C, na.rm = T),
          t_ann_max = max(t_avg_C, na.rm = T),
          prcp_median = median(prcp_daily_mm, na.rm = T),
          prcp_max = max(prcp_daily_mm, na.rm = T),
          prcp_min = min(prcp_daily_mm, na.rm = T))
```
...
aligned_data_stats %>%
  ggplot(aes(x = prcp_median, y = abundance_qty, color = site_name)) +
  geom_point() +
  geom_smooth(method = "lm") +
...
```{r}
#Mackenzie's attempt at statistics: 
