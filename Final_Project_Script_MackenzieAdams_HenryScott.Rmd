...
```{r}
# I know its easier to write seperate calls for each river but incase you were curious, heres it in 1 function

lotta_g <-
  function(f) {
    f %>% 
      ggplot(aes(x = Date, y = Flow)) +
      geom_line(color = rand_color(1)) + 
      theme_minimal() +
...
names(huuu) <- c("Dungeness", "Elwha", "Skokomish")
...
names(fish_mlr_wcs_summ) <- c("Dunguness River", "Elwha River", "Skokomish River")

wcs_pvals <-
  lapply(fish_mlr_wcs_summ, function(s) {
  coef(s)[,4]
})
...
p <- ggplot(df, aes(x = month_day, y = Flow, color = title_clean)) +
  geom_line() +
...
animate(p, width = 800, height = 400, fps = 5)
